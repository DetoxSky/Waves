...
public class HardFinalBoss extends GameObject{
	...
	public HardFinalBoss(int x, int y, ID id, Handler handler){
		super(x, y, id);
		...
		try {
		    img6 = ImageIO.read(new File("res/big-spider-face.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void tick(){
		...
		if(timer2 <= 0){
			...
			if(spawn >= 70 && spawn <= 85)
				handler.addObject(new HardFinalBossEnemy3((int)x + 48, (int)y + 48, ID.HardFatEnemy, handler));
		}
